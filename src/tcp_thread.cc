...
int tcp_open_th(int PORT,char* hostname){
  int sd;
  printf(" tcp_open_th:: PORT=%d HOST=%s \n",PORT,hostname);
...
int open_local_th(int PORT){                      /*--- LISTEN !!! -----*/
  ...
  struct sockaddr_in sinn;
    int on = 1;
    int	sd;
    printf(" open_local_th:: PORT=%d \n",PORT);
    /* get an internet domain socket */
    if ((sd = socket(AF_INET, SOCK_STREAM, 0)) == -1) {
	perror("socket");
	return -1;
    }
    printf("tcplib::open_local_th:: setsockopt(sock=%d) SO_REUSEADDR  !!!\n",sd);
    ...
    printf("tcplib::open_local_th::  bind socket=%d  !!!\n",sd);
...
int tcp_get_th(int sd_current,int *DATA,int lenDATA){
  ...
  char *rcv;
      //-----------------------------------------------------
  //printf("tcp_get_th(sd_current=%d DATA=%d lenDATA=%d)\n",sd_current,DATA[0],lenDATA);
      nleft=lenDATA;
      rcv=(char*)DATA;
      while(nleft>0){
...
int tcp_listen3(int sd, char* host_name, int len, int *sd_current){
  ...
  printf("listen3:: hp->h_name:: rem_port=%d \n",rem_port);

  printf("listen3:: try get host by addr %s \n",inet_ntoa(pin.sin_addr));
...
int tcp_close_th(int sd){
    close(sd);  
    printf("TCPLIB::close_port sd=%d \n",sd);
    return 0;
}
