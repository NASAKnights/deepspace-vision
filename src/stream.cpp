...
int maxV = 255;
*/
int minR = 103;//131  // latest:103 255, 144, 255, 0, 255
int maxR = 255;
int minG = 127;//128
int maxG = 282;
int minB = 110;//92
...
Mat ThresholdImage(Mat original)
{
  ...
  //inRange(original, Scalar(0, 0, 0), Scalar(255, 255, 255), thresholded);
 ...
 while (true)
      ...
      if(!frame.empty() && newFrame) //check it
	{
	  ...
	  if (nt==2) { 
	    ...
	    if (qdebug == -1) cout << "failed nt = " << nt << endl;
	    position.x=-1;
	    position.y=0;
	    position.z=-1;
	    position.angle=0;
	  ...
	  if(SHOWT)
        ...
        if (0) { // #stream
			...
			bool isOutputColored=false;
                        VideoWriter outStream("mjpeg/out.mjpg", CV_FOURCC('M','J','P','G'), 2, Size(FrameWidth, FrameHeight), isOutputColored);
                        if (outStream.isOpened()){
                             //   outStream.write(img);
                                outStream.write(thresholded);
                        } else {
                                cout<<"ERROR: Can't write to "<<"out File"<<"!\n";
                                return -1;
                        }
        }
